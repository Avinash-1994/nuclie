...
- **CLI & Config**: Loads environment, parses `sparx.config.js`, and manages the build/dev lifecycle.
...
Sparx ensures 100% deterministic builds by calculating a **Global Fingerprint** before every execution.

- **Storage**: Highly efficient SQLite database (`.sparx_cache/build.db`).
- **Fingerprint Scope**:
