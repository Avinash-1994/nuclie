# Nuclie Core Contract (v1.x)

## 1. Unified Pipeline Specification
Nuclie operates on a strictly ordered **10-stage pipeline**. Any modification to the core engine must preserve this sequence.
...
Nuclie provides a compatibility layer for the ecosystem. However, this is strictly limited to:
- **Stateless Transforms**: Mapping Rollup `transform` or Webpack `loader` to Nuclie's `transformModule` hook.
- **Stateless Resolvers**: Mapping standard `resolveId` hooks.
- **Output Rendering**: Basic `renderChunk` support.

*Plugins requiring deep access to Nuclie's Internal Graph API are NOT supported.*

---

## 4. Determinism Guarantee
In `ci` mode, Nuclie asserts that identical inputs MUST produce identical `BuildFingerprint` outputs.
