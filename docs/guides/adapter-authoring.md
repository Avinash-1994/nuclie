# Sparx Adapter Authoring Guide

**Module**: 13 (Phase H.5)  
**Status**: ✅ Active

This guide explains how to build a production-grade framework adapter for Sparx.

---

## 🏗️ Mental Model

An Sparx Adapter is an **isolated bridge** between Sparx's core and a framework's internal toolchain (e.g., Vite, esbuild, SWC). 
...
1.  **`init(options)`**: Receives the user's `sparx.config.js` and sets up the internal toolchain.
...
- [ ] Passes `npx tsx scripts/verify.ts` in the Sparx root.
