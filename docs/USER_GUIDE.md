...
Basic workflow:
- **Dev**: Run `npx urja dev`, see browser update via sub-100ms HMR.
- **Audit**: Run `npx urja verify` to check project health.
- **Build**: Run `npx urja build` for a production-ready bundle.
- **Analyze**: Run `npx urja analyze` to see bundle composition.

Files and folders you need to know
- `src/` — Put your app code here.
- `public/` — Static files like `index.html`.
- `build_output/` — Optimized production files.
- `.urja_cache/` — Native build database (SQLite).
- `urja.config.js` — Standard ESM configuration file.
...
Config file (`urja.config.js`) example:
```javascript
export default {
  entry: ['src/main.tsx'],
  mode: 'production',
