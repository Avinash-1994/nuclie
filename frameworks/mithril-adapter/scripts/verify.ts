import { MithrilAdapter } from '../src/index.js';
...
async function verifyApp(appName: string, checkFn: (output: AdapterOutput) => void) {
    ...
    const adapter = new MithrilAdapter();
    await adapter.init({
        root: testAppPath,
...
async function run() {
    try {
        await verifyApp('app-hello', (output) => {
            ...
            // Mithril usage: m(...)
            if (!jsFile || !jsFile.source.toString().includes('m(')) {
                throw new Error('JS output missing Mithril m() calls');
            }
        });
