...
/**
 * Module 3: Elite DX - Create-Sparx Test
 * Validates Day 17 Template Generation
 */

import * as fs from 'fs';
import * as path from 'path';
import { main } from '../src/create-sparx/cli.js'; // We might need to mock process.argv

const TEST_DIR = path.resolve('.test_create_sparx');
...
async function runTest() {
    console.log('🧪 Testing create-sparx CLI...');
    await setup();
    ...
    try {
        ...
        if (!fs.existsSync(path.join(projDir, 'sparx.config.ts'))) throw new Error('Config missing');
