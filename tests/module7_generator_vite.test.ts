...
<html>
  <body>
    ...
    it('should generate urja.config.ts in dry-run mode', async () => {
        ...
        expect(result.files).toContain('urja.config.ts');

        // Should NOT create file in dry-run
        expect(fs.existsSync(path.join(FIXTURE_DIR, 'urja.config.ts'))).toBe(false);
    });

    it('should generate urja.config.ts with correct framework', async () => {
        const analyzer = new MigrationAnalyzer(FIXTURE_DIR);
        const plan = await analyzer.analyze();
        ...
        const configPath = path.join(FIXTURE_DIR, 'urja.config.ts');
    ...
    it('should update package.json with Urja scripts', async () => {
        const analyzer = new MigrationAnalyzer(FIXTURE_DIR);
        const plan = await analyzer.analyze();
        ...
        expect(pkg.scripts.dev).toBe('urja dev');
        expect(pkg.scripts.build).toBe('urja build');
        expect(pkg.devDependencies.urja).toBe('^2.0.0');
    });
    ...
    it('should generate MIGRATION_REPORT.md', async () => {
        ...
        expect(content).toContain('# Urja Migration Report');
    ...
    it('should create backup directory when backup is enabled', async () => {
        ...
        const backupDir = path.join(FIXTURE_DIR, '.urja-migration-backup');
