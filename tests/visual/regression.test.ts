/**
 * Visual Regression Tests for Urja Dev Server UI
 * 
...
test.beforeAll(async () => {
    // Create test project
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'urja-visual-test-'));
    ...
    fs.writeFileSync(
        ...
        root.innerHTML = '<h1>Hello, Urja!</h1>';
        `
    );
...
test.describe('Visual Regression: Error Overlay', () => {
    ...
    test('should display error overlay with correct styling', async ({ page }) => {
        ...
        await page.waitForSelector('[data-urja-error-overlay]', { timeout: 5000 })
            .catch(() => {
        ...
        const overlay = await page.$('[data-urja-error-overlay]');
        if (overlay) {
...
test.describe('Visual Regression: Build Dashboard', () => {
    ...
    test('should display build progress', async ({ page }) => {
        ...
        const dashboard = await page.$('[data-urja-dashboard]')
            .catch(() => null);
