...
async function runOverlayTest() {
    ...
    // Verify UI
    const overlay = document.querySelector('urja-error-overlay');
    ...
    (window as any).__URJA_OVERLAY__.reportBuildError({
    ...
    for (let i = 0; i < 100; i++) {
        (window as any).__URJA_OVERLAY__.reportBuildError({ message: `Err ${i}` });
