...
async function runOverlayTest() {
    ...
    // Verify UI
    const overlay = document.querySelector('sparx-error-overlay');
    ...
    (window as any).__SPARX_OVERLAY__.reportBuildError({
    ...
    for (let i = 0; i < 100; i++) {
        (window as any).__SPARX_OVERLAY__.reportBuildError({ message: `Err ${i}` });
