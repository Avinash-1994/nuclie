...
/**
 * Module 3: Elite DX - Create-Urja Test
 * Validates Day 17 Template Generation
 */

import * as fs from 'fs';
import * as path from 'path';
import { main } from '../src/create-urja/cli.js'; // We might need to mock process.argv

const TEST_DIR = path.resolve('.test_create_urja');
...
async function runTest() {
    console.log('🧪 Testing create-urja CLI...');
    await setup();
    ...
    try {
        ...
        if (!fs.existsSync(path.join(projDir, 'urja.config.ts'))) throw new Error('Config missing');
