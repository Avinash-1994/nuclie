...
import { sparxSvelte } from '../src/plugins/frameworks/svelte.js';
import { sparxSolid } from '../src/plugins/frameworks/solid.js';
import { sparxLit } from '../src/plugins/frameworks/lit.js';
import { strict as assert } from 'assert';

async function testSvelteBasicCompilation() {
    console.log('\n[Test 1] Svelte - Basic Component Compilation');

    const plugin = sparxSvelte({ development: true });
...
async function testSvelteProductionMode() {
    ...
    const plugin = sparxSvelte({ development: false, hmr: false });
...
async function testSolidBasicJSX() {
    ...
    const plugin = sparxSolid({ development: true });
...
async function testSolidProductionMode() {
    ...
    const plugin = sparxSolid({ development: false, hmr: false });
...
async function testLitBasicComponent() {
    ...
    const plugin = sparxLit({ development: true });
...
async function testLitProductionMode() {
    ...
    const plugin = sparxLit({ development: false, hmr: false });
...
async function testFrameworkDetection() {
    ...
    const sveltePlugin = sparxSvelte();
    const solidPlugin = sparxSolid();
    const litPlugin = sparxLit();
