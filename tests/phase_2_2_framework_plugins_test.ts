...
import { nuclieReact } from '../src/plugins/frameworks/react.js';
import { nuclieVue } from '../src/plugins/frameworks/vue.js';
import { strict as assert } from 'assert';

async function testReactBasicTransform() {
    console.log('\n[Test 1] React - Basic JSX Transform');

    const plugin = nuclieReact({ development: true, fastRefresh: false });
...
async function testReactCSSTracking() {
    ...
    const plugin = nuclieReact({ development: true });
...
async function testReactComponentDetection() {
    ...
    const plugin = nuclieReact({ development: true, fastRefresh: true });
...
async function testVueBasicSFC() {
    ...
    const plugin = nuclieVue({ development: true });
...
async function testReactProductionMode() {
    ...
    const plugin = nuclieReact({ development: false, fastRefresh: false });
...
async function testVueProductionMode() {
    ...
    const plugin = nuclieVue({ development: false, hmr: false });
