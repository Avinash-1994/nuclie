...
/**
 * Module 3: Elite DX - Create-Nuclie Test
 * Validates Day 17 Template Generation
 */

import * as fs from 'fs';
import * as path from 'path';
import { main } from '../src/create-nuclie/cli.js'; // We might need to mock process.argv

const TEST_DIR = path.resolve('.test_create_nuclie');
...
async function runTest() {
    console.log('🧪 Testing create-nuclie CLI...');
    await setup();
    ...
    try {
        ...
        if (!fs.existsSync(path.join(projDir, 'nuclie.config.ts'))) throw new Error('Config missing');
