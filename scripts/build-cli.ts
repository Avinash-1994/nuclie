...
async function bundleCLI() {
    console.log('🚀 Bundling Sparx CLI for production performance...');
    ...
    try {
        const result = await build({
            ...
            banner: {
                js: `import { createRequire as __sparx_createRequire } from 'module';
const require = __sparx_createRequire(import.meta.url);
`,
            },
            logLevel: 'info',
            metafile: true,
        ...
        const nativeModulePath = path.resolve(process.cwd(), 'sparx_native.node');
        if (fs.existsSync(nativeModulePath)) {
            fs.copyFileSync(nativeModulePath, path.join(process.cwd(), 'dist/sparx_native.node'));
            console.log('✅ Native module [sparx_native.node] copied to dist/');
        }
