...
async function printBuildSummary(outDir: string, elapsed: number) {
  ...
  const EXCLUDE_NAMES = ['sparx-sbom', 'sparx-csp', 'sparx-sri', 'sparx-headers', '_headers', '.htaccess'];
    ...
    try {
      ...
      if (args.watch) {
        ...
        const rebuild = async (filePath: string) => {
          console.log(`  [sparx] Changed: ${path.relative(root, filePath)}`);
