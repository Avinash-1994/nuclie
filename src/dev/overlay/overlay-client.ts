...
function getOverlay() {
    if (!overlayInstance || !overlayInstance.isConnected) {
        overlayInstance = document.createElement('sparx-error-overlay');
...
(window as any).__SPARX_OVERLAY__ = {
...
export function activateOverlay() {
    ...
    console.log('[Sparx] Error Overlay Active');
}
