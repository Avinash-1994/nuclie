...
export async function startDevServer(cfg: BuildConfig) {
    ...
    const server = http.createServer(async (req, res) => {
        if (features && (server as any).__sparx_handler) {
            return (server as any).__sparx_handler(req, res);
        }
        ...
        if (!expectsHtml) {
            await initPromise;
            if ((server as any).__sparx_handler) {
                return (server as any).__sparx_handler(req, res);
            }
        }
    ...
    await new Promise<void>((resolve, reject) => {
        ...
        server.listen(port, host, () => {
            ...
            // Futuristic Sparx Branding (Engineering First)
            console.log(`\x1b[36m   ⚡ SPARX \x1b[90mv${pkgVersion}\x1b[0m`);
