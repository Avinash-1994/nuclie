...
// Main Flow
export async function createNuclieProject(initialName?: string) {
    console.log(kleur.bold().magenta("\n" + "=".repeat(40)));
    console.log(kleur.bold().white("  ⚡ NUCLIE: THE NEXT-GEN BUILD PROJECT  "));
    console.log(kleur.bold().magenta("=".repeat(40) + "\n"));

    const name = initialName || await text('Project Name:', 'my-nuclie-app');
...
async function generateProject(config: ProjectConfig) {
    ...
    const nuclieConfig = generateNuclieConfig(config);
    const configFileName = config.language === 'TypeScript' ? 'nuclie.config.ts' : 'nuclie.config.js';
    await fsPromises.writeFile(path.join(projectPath, configFileName), nuclieConfig);
    ...
    // Nuclie handles them internally if missing.
...
function generatePackageJson(config: ProjectConfig) {
    ...
    pkg.devDependencies[`@nuclie/framework-${frameworkKey}`] = 'latest';
...
<html lang="en">
  <head>
    ...
    <title>${config.name} | Nuclie</title>
  </head>
  <body>
    <div id="root"></div>
...
`);
    ...
    await fsPromises.writeFile(path.join(publicDir, 'nuclie.svg'), `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><circle cx="50" cy="50" r="40" fill="#38BDF8" /><path d="M50 20 L80 80 L20 80 Z" fill="white" /></svg>`);
}
...
function generateReadme(config: ProjectConfig) {
    ...
    if (config.projectType.includes('Micro-Frontend')) {
        mfeWarning = `
> ⚠️ **Micro-Frontend Note**: Only one framework per micro-frontend is supported in the current version of Nuclie.
`;
    }
...
## Architecture
- **Adapter**: ${config.framework === 'Vanilla' ? 'None' : `@nuclie/framework-${config.framework.toLowerCase()}`}
- **Config**: \`nuclie.config.${config.language === 'TypeScript' ? 'ts' : 'js'}\`

Built with energy, powered by Nuclie.
`;
}
...
export class NuclieApp extends LitElement {
  render() {
    return html\`<h1>Hello from Lit + Nuclie</h1>\`;
  }
}
document.getElementById('root')!.innerHTML = '<nuclie-app></nuclie-app>';`;
        case 'Alpine':
...
document.querySelector('#root')!.innerHTML = \`
  <div class="\${styles.container}">
    <h1 class="\${styles.title}">⚡ Vanilla JS + Nuclie</h1>
...
document.querySelector('#root')!.innerHTML = \`
  <div style="text-align: center; font-family: sans-serif;">
    <h1>⚡ Vanilla JS + Nuclie</h1>
    <p>Zero dependencies. Pure speed.</p>
  </div>
\`;`;
        default:
            return `import '${styleImportSource}';
document.getElementById('root')!.innerHTML = '<h1>Hello Nuclie</h1>';`;
    }
}
...
export default function App() {
  return (
    <div className={styles.container}>
      <h1 className={styles.title}>Nuclie + ${config.framework} + CSS Modules</h1>
