/**
 * Nuclie Plugin Governance & Stability System
 * 
...
/**
 * Validates a plugin against Nuclie's strict governance rules.
 * 
 * Rules:
...
export function validatePlugin(plugin: any): GovernanceResult {
    ...
    if (!/^(@[a-z0-9-]+\/)?[a-z0-9-]+$/.test(plugin.name)) {
        result.warnings.push(`Plugin name "${plugin.name}" should be kebab-case (e.g. nuclie-plugin-foo).`);
    }
...
export function isVerified(name: string): boolean {
    return VERIFIED_PLUGINS.has(name) || name.startsWith('@nuclie/');
}
