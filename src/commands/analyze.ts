...
async function generateChromeTrace(events: any[]) {
    ...
    const tracePath = path.join(process.cwd(), 'sparx-trace.json');
...
<head>
    ...
    <title>Sparx Premium Bundle Analyzer</title>
...
<body>
    ...
    const reportPath = path.join(process.cwd(), 'sparx-report.html');
