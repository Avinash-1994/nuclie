/**
 * sparx migrate — config and plugin migration from older Sparx versions
 */
...
const MIGRATIONS = [
  {
    id: 'M01',
    title: 'Rename nuclie.config.* → sparx.config.*',
    detect: (root: string) =>
    ...
    apply: (root: string) => {
      ...
      const newName = `sparx.config${ext}`;
  ...
  {
    id: 'M02',
    title: 'Rewrite @nuclie/* imports → @sparx/* in package.json',
    detect: (root: string) => {
    ...
    apply: (root: string) => {
      ...
      const content = fs.readFileSync(pkgPath, 'utf8');
      const updated = content.replace(/@nuclie\//g, '@sparx/');
      fs.writeFileSync(pkgPath, updated, 'utf8');
      console.log('  ✅ Rewrote @nuclie/* → @sparx/* in package.json');
    }
  },
  {
    id: 'M03',
    ...
    apply: (root: string) => {
      ...
      for (const file of files) {
        ...
        if (content.includes('@nuclie/')) {
          fs.writeFileSync(file, content.replace(/@nuclie\//g, '@sparx/'), 'utf8');
          count++;
        }
      }
      console.log(`  ✅ Rewrote @nuclie/* → @sparx/* in ${count} source file(s)`);
    }
  }
];
...
function findSourceFiles(dir: string): string[] {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory() && !['node_modules', '.sparx', 'dist', 'build_output'].includes(entry.name)) {
