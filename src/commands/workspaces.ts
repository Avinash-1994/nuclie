...
export async function startWorkspaceOrchestrator(rootDir: string) {
    console.log('\n🚀 Starting Sparx Monorepo Workspace Orchestrator...');
        ...
        try {
            ...
            for (const entry of entries) {
                if (entry.isDirectory() && entry.name !== 'node_modules' && !entry.name.startsWith('.')) {
                    ...
                    try {
                        await fs.access(path.join(fullPath, 'sparx.config.js'));
    ...
    if (projectPaths.length === 0) {
        console.error('❌ No Sparx projects (with sparx.config.js) found in subdirectories!');
        process.exit(1);
    }
