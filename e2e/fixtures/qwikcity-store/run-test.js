...
await (async function testServerActions() {
  const cartPayload = { productId: 'sparx-pro-kit', quantity: 2 };
...
await (async function testColdStart() {
  ...
  (coldMs < gate ? printPass : printFail)(
    ...
    `Environment: ${env}`,
    `[sparx] adapter: qwik-city in output: yes`,
...
await (async function testHmr() {
  ...
  const changeDesc = 'appended comment // sparx-hmr-ts (triggers module invalidation)';
...
log('');
log('┌─────────────────────────────────────────────┐');
log('│ SPARX — PHASE 2.5 QWIK CITY COMPLETE       │');
log('│                                             │');
...
log(`│ [sparx] adapter: qwik-city confirmed        │`);
