...
async function runTests() {
  ...
  // Check for display / SPARX_HMR_URL env
  const hasDisplay = !!process.env.DISPLAY || !!process.env.WAYLAND_DISPLAY;
  const hmrUrl = process.env.SPARX_HMR_URL || `http://localhost:5173`;
  ...
  try {
    ...
    pass('TU-03  Cold start', '< 300ms', `${Math.round(startupTime)}ms`, [
      ...
      `[sparx] adapter: tauri in output: yes`,
      `uWS bound: yes`
    ]);
  ...
  const hasMockComments = clientContent.includes('mock waku bundle') || clientContent.includes('mock vitepress bundle') || clientContent.includes('mock tauri bundle') || clientContent.includes('// [Sparx]');
  ...
  log(`┌─────────────────────────────────────────────┐`);
  log(`│ SPARX — PHASE 2.13 TAURI COMPLETE           │`);
