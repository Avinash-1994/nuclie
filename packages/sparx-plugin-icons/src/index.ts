/**
 * @sparx/plugin-icons — 100,000+ Icons, Zero Bundle Bloat
...
export interface IconsPluginOptions {
  ...
  /** Cache directory (default: .sparx/cache/icons) */
  cacheDir?: string;
}
...
export function icons(options: IconsPluginOptions = {}) {
  const {
    compiler = 'react',
    defaultClass = 'sparx-icon',
    cacheDir = '.sparx/cache/icons',
    customCollections = {},
  } = options;
  ...
  return {
    ...
    async load(id: string) {
      ...
      if (!svg) {
        console.warn(`[sparx:icons] Icon not found: ${collection}/${name}`);
