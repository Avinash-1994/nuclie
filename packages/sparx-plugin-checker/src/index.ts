/**
 * @sparx/plugin-checker — TypeScript, ESLint, Vue type errors in terminal
 * Replaces: vite-plugin-checker
 * Permissions: exec:spawn
 *
 * Runs TS/ESLint in worker threads — never blocks main dev server thread.
 * Errors appear in terminal AND the Sparx error overlay.
 */
...
export function checker(options: CheckerOptions = {}) {
  ...
  async function runChecks(isProduction: boolean): Promise<string[]> {
    ...
    if (typescript) {
      ...
      if (errors.length) console.error(`[sparx:checker] TypeScript: ${errors.length} error(s)`);
    }
    ...
    if (eslint && changedFiles.size > 0) {
      ...
      allErrors.push(...errors);
      if (errors.length) console.error(`[sparx:checker] ESLint: ${errors.length} issue(s)`);
    }
  ...
  return {
    ...
    async buildStart() {
      ...
      if (errors.length > 0) {
        throw new Error(
          `[sparx:checker] Build aborted: ${errors.length} type/lint error(s) found.\n` +
