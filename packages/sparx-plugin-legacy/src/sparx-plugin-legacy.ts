/**
 * @sparx/plugin-legacy — Support older browsers without hurting modern ones
...
function generatePolyfillBundle(): string {
  return `// @sparx/plugin-legacy polyfills
// Object.assign
...
// Promise (minimal)
if (typeof Promise === 'undefined') { console.warn('[sparx:legacy] Promise polyfill not included — use core-js'); }
// Array.from
...
export function legacy(options: LegacyOptions = {}) {
  ...
  return {
    ...
    async closeBundle() {
      ...
      console.info(`[sparx:legacy] ✅ Legacy bundle support injected (Safari 10 fix: ${safari10})`);
    },
  };
}
