/**
 * @nuclie/plugin-env — Environment Variables, Done Right
...
export function env(options: EnvPluginOptions = {}) {
  ...
  return {
    ...
    configResolved(config: any) {
      ...
      if (missing.length > 0) {
        throw new Error(
          `[nuclie:env] Missing required environment variable(s): ${missing.join(', ')}\n` +
    ...
    transform(code: string, id: string) {
      ...
      for (const [key, value] of Object.entries(envVars)) {
        ...
        if (isSecret(value) && !safe.includes(key)) {
          console.warn(`[nuclie:env] Blocking secret-pattern var "${key}" from bundle.`);
          continue;
        }
