/**
 * @nuclie/plugin-pwa — Progressive Web App, one plugin, full offline support
...
function generateServiceWorker(precacheUrls: string[], options: PWAOptions): string {
  ...
  return `// @nuclie/plugin-pwa — Generated Service Worker
const CACHE_VERSION = 'nuclie-v${cacheVersion}';
...
export function pwa(options: PWAOptions) {
  ...
  return {
    ...
    async closeBundle() {
      ...
      console.info(`[nuclie:pwa] ✅ PWA manifest + service worker generated (${assets.length} assets precached)`);
    },
