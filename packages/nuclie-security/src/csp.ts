...
export function generateCSP(distDir: string, config: CSPConfig = {}): CSPResult {
  ...
  // Write output files
  if (fs.existsSync(distDir)) {
    fs.writeFileSync(path.join(distDir, 'nuclie-csp.txt'), `Content-Security-Policy: ${header}\n`, 'utf8');
    fs.writeFileSync(path.join(distDir, 'nuclie-csp-meta.html'), metaTag + '\n', 'utf8');
    console.info(`[nuclie:security] CSP generated → ${path.join(distDir, 'nuclie-csp.txt')}`);
  }
