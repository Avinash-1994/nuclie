 ...
 * Plugins must declare permissions in package.json under nuclie.permissions.
 */
...
function logViolation(pluginName: string, operation: string, logDir?: string): void {
  ...
  console.warn(`[nuclie:security] ${msg}`);
  if (logDir) {
...
export function loadPluginPermissions(
  ...
  try {
    ...
    const declared: PluginPermission[] = pkg?.nuclie?.permissions ?? [];
    ...
    get(target, prop) {
      ...
      if (prop === 'env' || prop === 'process') {
        if (!declared.includes('env:read')) {
          ...
          if (mode === 'production') {
            throw new Error(`[nuclie:security] Plugin "${name}" requires env:read permission.`);
          }
...
/** List installed Nuclie plugins with their declared permissions for audit. */
