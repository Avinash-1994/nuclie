...
export function guardEnvAccess(
  ...
  if (!hasEnvReadPermission && !isSafe) {
    console.warn(
      `[nuclie:security] Plugin attempted to read env var "${key}" without env:read permission. Value blocked.`
    );
    return undefined;
  }
...
export function validatePath(
  ...
  console.error(
    `[nuclie:security] Path traversal attempt blocked.\n` +
...
export function validateSymlink(
  ...
  try {
    ...
    console.warn(
      `[nuclie:security] Symlink outside project root blocked (policy: ${policy}).\n` +
      `  Symlink: ${symlinkPath}\n` +
