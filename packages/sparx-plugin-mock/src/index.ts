/**
 * @sparx/plugin-mock — Mock API responses in dev, zero backend needed
...
export function mock(options: MockPluginOptions = {}) {
  ...
  if (!enable) {
    return { name: '@sparx/plugin-mock', configureServer() {} };
  }
  ...
  return {
    ...
    configResolved(config: any) {
      ...
      console.info(`[sparx:mock] Loaded ${handlers.size} mock route(s) from ${mockDir}/`);
    },
      ...
      if (server.watcher) {
        ...
        server.watcher.on('change', (file: string) => {
          if (file.startsWith(mockDir)) {
            handlers = loadMockHandlers(mockDir);
            console.info('[sparx:mock] Mock handlers reloaded.');
          }
        });
      }
      ...
      server.middlewares?.use(async (req: any, res: any, next: () => void) => {
        ...
        req.on('end', async () => {
          ...
          try {
            ...
            console.error('[sparx:mock] Handler error:', err);
            next();
          }
        });
      });
