 ...
 * Generates dist/sparx-sbom.json listing every npm + Rust dep
 ...
 * @param projectRoot – root of the Sparx project
 * @param resolvedModules – list of resolved npm package names (from dep graph)
 */
export async function generateSBOM(projectRoot, resolvedModules = []) {
...
/** Write the SBOM to dist/sparx-sbom.json */
export function writeSBOM(report, distDir) {
    const outPath = path.join(distDir, 'sparx-sbom.json');
    fs.mkdirSync(distDir, { recursive: true });
    fs.writeFileSync(outPath, JSON.stringify(report, null, 2), 'utf8');
    console.info(`[sparx:security] SBOM written → ${outPath} (${report.components.length} components)`);
}
function normalizeLicenses(raw) {
    ...
    if (Array.isArray(raw))
        return raw.map((l) => l?.type ?? String(l));
